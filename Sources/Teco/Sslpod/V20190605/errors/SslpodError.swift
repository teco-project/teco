...
/// Service error type returned by `Sslpod`.
public protocol TCSslpodErrorType: TCPlatformErrorType {
    /// Get the service error as ``TCSslpodError``.
    ///
    /// - Returns: ``TCSslpodError`` that holds the same error code and context.
    func asSslpodError() -> TCSslpodError
}

public struct TCSslpodError: TCSslpodErrorType {
    enum Code: String {
    ...
    /// Error domains affliated to ``TCSslpodError``.
    public static var domains: [TCErrorType.Type] {
        [FailedOperation.self, InvalidParameter.self, InvalidParameterValue.self, LimitExceeded.self, ResourceNotFound.self]
    }
