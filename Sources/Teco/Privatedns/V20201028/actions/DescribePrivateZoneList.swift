...
extension Privatedns {
    ...
    public struct DescribePrivateZoneListRequest: TCPaginatedRequest {
        ...
        public func getNextPaginatedRequest(with response: DescribePrivateZoneListResponse) -> DescribePrivateZoneListRequest? {
            guard !response.getItems().isEmpty else {
                return nil
            }
            return DescribePrivateZoneListRequest(offset: (self.offset ?? 0) + .init(response.getItems().count), limit: self.limit, filters: self.filters)
        }
    }
