...
extension Iotvideo {
    ...
    /// 本接口（DisableDevice）用于禁用设备，可进行批量操作，每次操作最多100台设备。
    @inlinable
    public func disableDevice(tids: [String], logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DisableDeviceResponse {
        try await self.disableDevice(DisableDeviceRequest(tids: tids), logger: logger, on: eventLoop)
    }
}
