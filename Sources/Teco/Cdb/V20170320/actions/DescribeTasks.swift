...
extension Cdb {
    ...
    /// 本接口(DescribeTasks)用于查询云数据库实例任务列表。
    @inlinable
    public func describeTasksPaginated(_ input: DescribeTasksRequest, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<(Int64?, [TaskDetail])> {
    ...
    /// 本接口(DescribeTasks)用于查询云数据库实例任务列表。
    ///
    /// - Returns: `AsyncSequence`s of `TaskDetail` and `DescribeTasksResponse` that can be iterated over asynchronously on demand.
    @inlinable
