...
extension Wedata {
    ...
    public struct BatchStopIntegrationTasksRequest: TCRequestModel {
        /// 任务id
    ...
    public struct BatchStopIntegrationTasksResponse: TCResponseModel {
        /// 操作成功的任务数
    ...
    public func batchStopIntegrationTasks(taskIds: [String], taskType: Int64, projectId: String, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<BatchStopIntegrationTasksResponse> {
        let input = BatchStopIntegrationTasksRequest(taskIds: taskIds, taskType: taskType, projectId: projectId)
        return self.client.execute(action: "BatchStopIntegrationTasks", region: region, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
    ...
    public func batchStopIntegrationTasks(taskIds: [String], taskType: Int64, projectId: String, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> BatchStopIntegrationTasksResponse {
        let input = BatchStopIntegrationTasksRequest(taskIds: taskIds, taskType: taskType, projectId: projectId)
        return try await self.client.execute(action: "BatchStopIntegrationTasks", region: region, serviceConfig: self.config, input: input, logger: logger, on: eventLoop).get()
    }
}
