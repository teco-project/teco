...
extension Wedata {
    ...
    /// 过滤条件】 {模版名称Name,支持模糊匹配} {模版类型type，1.系统模版 2.自定义模版} {质量检测维度QualityDims, 1.准确性 2.唯一性 3.完整性 4.一致性 5.及时性 6.有效性} 【排序字段】 { 引用数排序类型CitationOrderType，根据引用数量排序 ASC DESC}
    @inlinable
    public func describeRuleTemplatesByPagePaginator(_ input: DescribeRuleTemplatesByPageRequest, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> (results: TCClient.Paginator<DescribeRuleTemplatesByPageRequest, DescribeRuleTemplatesByPageResponse>.ResultSequence, responses: TCClient.Paginator<DescribeRuleTemplatesByPageRequest, DescribeRuleTemplatesByPageResponse>.ResponseSequence) {
        let results = TCClient.Paginator<DescribeRuleTemplatesByPageRequest, DescribeRuleTemplatesByPageResponse>.ResultSequence(input: input, region: region, command: self.describeRuleTemplatesByPage, logger: logger, on: eventLoop)
        let responses = TCClient.Paginator<DescribeRuleTemplatesByPageRequest, DescribeRuleTemplatesByPageResponse>.ResponseSequence(input: input, region: region, command: self.describeRuleTemplatesByPage, logger: logger, on: eventLoop)
