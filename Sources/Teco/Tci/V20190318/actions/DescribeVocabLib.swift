...
extension Tci {
    ...
    public func describeVocabLib(region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DescribeVocabLibResponse> {
        let input = DescribeVocabLibRequest()
        return self.client.execute(action: "DescribeVocabLib", region: region, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
    ...
    public func describeVocabLib(region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DescribeVocabLibResponse {
        let input = DescribeVocabLibRequest()
        return try await self.client.execute(action: "DescribeVocabLib", region: region, serviceConfig: self.config, input: input, logger: logger, on: eventLoop).get()
    }
}
