...
extension Cloudstudio {
    ...
    /// 运行空间
    @inlinable @discardableResult
    public func runWorkspace(spaceKey: String, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<RunWorkspaceResponse> {
        self.runWorkspace(.init(spaceKey: spaceKey), region: region, logger: logger, on: eventLoop)
    }

    /// 运行空间
    @inlinable @discardableResult
    public func runWorkspace(spaceKey: String, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> RunWorkspaceResponse {
        try await self.runWorkspace(.init(spaceKey: spaceKey), region: region, logger: logger, on: eventLoop)
    }
}
