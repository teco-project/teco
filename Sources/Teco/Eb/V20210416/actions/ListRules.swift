...
extension Eb {
    ...
    public struct ListRulesRequest: TCPaginatedRequest {
        ...
        public func getNextPaginatedRequest(with response: ListRulesResponse) -> ListRulesRequest? {
            ...
            return ListRulesRequest(eventBusId: self.eventBusId, orderBy: self.orderBy, limit: self.limit, offset: (self.offset ?? 0) + .init(response.getItems().count), order: self.order)
        }
    }
    ...
    public func listRules(eventBusId: String, orderBy: String? = nil, limit: Int64? = nil, offset: Int64? = nil, order: String? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<ListRulesResponse> {
        ...
        return self.client.execute(action: "ListRules", region: region, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
    ...
    public func listRulesPaginator(_ input: ListRulesRequest, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> (results: TCClient.Paginator<ListRulesRequest, ListRulesResponse>.ResultSequence, responses: TCClient.Paginator<ListRulesRequest, ListRulesResponse>.ResponseSequence) {
        let results = TCClient.Paginator<ListRulesRequest, ListRulesResponse>.ResultSequence(input: input, region: region, command: self.listRules, logger: logger, on: eventLoop)
        let responses = TCClient.Paginator<ListRulesRequest, ListRulesResponse>.ResponseSequence(input: input, region: region, command: self.listRules, logger: logger, on: eventLoop)
