...
extension Ecm {
    ...
    /// 修改监听器绑定的后端机器的转发权重。
    @inlinable
    public func modifyTargetWeight(loadBalancerId: String, listenerId: String, targets: [Target], weight: Int64? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture < ModifyTargetWeightResponse > {
        self.modifyTargetWeight(ModifyTargetWeightRequest(loadBalancerId: loadBalancerId, listenerId: listenerId, targets: targets, weight: weight), logger: logger, on: eventLoop)
    }
    ...
    /// 修改监听器绑定的后端机器的转发权重。
    @inlinable
    public func modifyTargetWeight(loadBalancerId: String, listenerId: String, targets: [Target], weight: Int64? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> ModifyTargetWeightResponse {
        try await self.modifyTargetWeight(ModifyTargetWeightRequest(loadBalancerId: loadBalancerId, listenerId: listenerId, targets: targets, weight: weight), logger: logger, on: eventLoop)
    }
}
