...
extension Bm {
    ...
    /// 创建黑石竞价实例
    @inlinable
    public func createSpotDevice(zone: String, computeType: String, osTypeId: UInt64, vpcId: String, subnetId: String, goodsNum: UInt64, spotStrategy: String, spotPriceLimit: Float? = nil, passwd: String? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<CreateSpotDeviceResponse> {
        self.createSpotDevice(CreateSpotDeviceRequest(zone: zone, computeType: computeType, osTypeId: osTypeId, vpcId: vpcId, subnetId: subnetId, goodsNum: goodsNum, spotStrategy: spotStrategy, spotPriceLimit: spotPriceLimit, passwd: passwd), logger: logger, on: eventLoop)
    }

    /// 创建黑石竞价实例
    @inlinable
    public func createSpotDevice(zone: String, computeType: String, osTypeId: UInt64, vpcId: String, subnetId: String, goodsNum: UInt64, spotStrategy: String, spotPriceLimit: Float? = nil, passwd: String? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> CreateSpotDeviceResponse {
        try await self.createSpotDevice(CreateSpotDeviceRequest(zone: zone, computeType: computeType, osTypeId: osTypeId, vpcId: vpcId, subnetId: subnetId, goodsNum: goodsNum, spotStrategy: spotStrategy, spotPriceLimit: spotPriceLimit, passwd: passwd), logger: logger, on: eventLoop)
    }
}
