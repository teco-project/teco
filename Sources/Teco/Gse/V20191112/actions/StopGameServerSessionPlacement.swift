...
extension Gse {
    ...
    /// 本接口（StopGameServerSessionPlacement）用于停止放置游戏服务器会话。
    @inlinable
    public func stopGameServerSessionPlacement(placementId: String, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> StopGameServerSessionPlacementResponse {
        try await self.stopGameServerSessionPlacement(StopGameServerSessionPlacementRequest(placementId: placementId), logger: logger, on: eventLoop)
    }
}
