...
extension Gse {
    ...
    /// 本接口（DescribeGameServerSessionPlacement）用于查询游戏服务器会话的放置。
    @inlinable
    public func describeGameServerSessionPlacement(placementId: String, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DescribeGameServerSessionPlacementResponse {
        try await self.describeGameServerSessionPlacement(DescribeGameServerSessionPlacementRequest(placementId: placementId), logger: logger, on: eventLoop)
    }
}
