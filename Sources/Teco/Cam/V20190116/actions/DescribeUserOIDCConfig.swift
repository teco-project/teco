...
extension Cam {
    ...
    public func describeUserOIDCConfig(region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DescribeUserOIDCConfigResponse> {
        let input = DescribeUserOIDCConfigRequest()
        return self.client.execute(action: "DescribeUserOIDCConfig", region: region, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
    ...
    public func describeUserOIDCConfig(region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DescribeUserOIDCConfigResponse {
        let input = DescribeUserOIDCConfigRequest()
        return try await self.client.execute(action: "DescribeUserOIDCConfig", region: region, serviceConfig: self.config, input: input, logger: logger, on: eventLoop).get()
    }
}
