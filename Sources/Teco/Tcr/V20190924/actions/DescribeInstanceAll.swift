...
extension Tcr {
    ...
    public struct DescribeInstanceAllRequest: TCPaginatedRequest {
        /// 实例ID列表(为空时，
        /// 表示获取账号下所有实例)
        ...
        public func makeNextRequest(with response: DescribeInstanceAllResponse) -> DescribeInstanceAllRequest? {
            guard !response.getItems().isEmpty else {
                return nil
            }
            return DescribeInstanceAllRequest(registryids: self.registryids, offset: (self.offset ?? 0) + .init(response.getItems().count), limit: self.limit, filters: self.filters, allRegion: self.allRegion)
        }
    }

    /// DescribeInstanceAll返回参数结构体
    public struct DescribeInstanceAllResponse: TCPaginatedResponse {
        /// 总实例个数
    ...
    public func describeInstanceAll(registryids: [String]? = nil, offset: Int64? = nil, limit: Int64? = nil, filters: [Filter]? = nil, allRegion: Bool? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DescribeInstanceAllResponse> {
        self.describeInstanceAll(.init(registryids: registryids, offset: offset, limit: limit, filters: filters, allRegion: allRegion), region: region, logger: logger, on: eventLoop)
    }
    ...
    public func describeInstanceAll(registryids: [String]? = nil, offset: Int64? = nil, limit: Int64? = nil, filters: [Filter]? = nil, allRegion: Bool? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DescribeInstanceAllResponse {
        try await self.describeInstanceAll(.init(registryids: registryids, offset: offset, limit: limit, filters: filters, allRegion: allRegion), region: region, logger: logger, on: eventLoop)
    }
