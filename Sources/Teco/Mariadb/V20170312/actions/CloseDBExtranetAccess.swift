...
extension Mariadb {
    ...
    /// 本接口(CloseDBExtranetAccess)用于关闭云数据库实例的外网访问。关闭外网访问后，外网地址将不可访问，查询实例列表接口将不返回对应实例的外网域名和端口信息。
    @inlinable
    public func closeDBExtranetAccess(instanceId: String, ipv6Flag: Int64? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> CloseDBExtranetAccessResponse {
        try await self.closeDBExtranetAccess(CloseDBExtranetAccessRequest(instanceId: instanceId, ipv6Flag: ipv6Flag), logger: logger, on: eventLoop)
    }
}
