...
extension Cvm {
    ...
    /// * 如果是系统盘，目前只支持扩容，不支持缩容。
    @inlinable
    public func resizeInstanceDisks(instanceId: String, dataDisks: [DataDisk]? = nil, forceStop: Bool? = nil, systemDisk: SystemDisk? = nil, resizeOnline: Bool? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> ResizeInstanceDisksResponse {
        try await self.resizeInstanceDisks(ResizeInstanceDisksRequest(instanceId: instanceId, dataDisks: dataDisks, forceStop: forceStop, systemDisk: systemDisk, resizeOnline: resizeOnline), logger: logger, on: eventLoop)
    }
}
