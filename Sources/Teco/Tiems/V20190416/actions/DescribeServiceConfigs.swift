...
extension Tiems {
    ...
    public struct DescribeServiceConfigsRequest: TCPaginatedRequest {
        ...
        public func makeNextRequest(with response: DescribeServiceConfigsResponse) -> DescribeServiceConfigsRequest? {
            ...
            return .init(filters: self.filters, offset: (self.offset ?? 0) + .init(response.getItems().count), limit: self.limit, order: self.order, orderField: self.orderField, pageByName: self.pageByName)
        }
    }
