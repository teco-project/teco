...
extension Vpc {
    ...
    /// 本接口会校验启用后，是否与已有路由冲突，如果冲突，则无法启用，失败处理。路由冲突时，需要先禁用与之冲突的路由，才能启用该路由。
    @inlinable
    public func enableRoutes(routeTableId: String, routeIds: [UInt64]? = nil, routeItemIds: [String]? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> EnableRoutesResponse {
        try await self.enableRoutes(EnableRoutesRequest(routeTableId: routeTableId, routeIds: routeIds, routeItemIds: routeItemIds), logger: logger, on: eventLoop)
    }
}
