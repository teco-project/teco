...
extension Vpc {
    ...
    /// 本接口（DescribeServiceTemplates）用于查询协议端口模板。
    @inlinable
    public func describeServiceTemplates(filters: [Filter]? = nil, offset: String? = nil, limit: String? = nil, needMemberInfo: Bool? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DescribeServiceTemplatesResponse> {
        self.describeServiceTemplates(.init(filters: filters, offset: offset, limit: limit, needMemberInfo: needMemberInfo), region: region, logger: logger, on: eventLoop)
    }
    ...
    /// 本接口（DescribeServiceTemplates）用于查询协议端口模板。
    @inlinable
    public func describeServiceTemplates(filters: [Filter]? = nil, offset: String? = nil, limit: String? = nil, needMemberInfo: Bool? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DescribeServiceTemplatesResponse {
        try await self.describeServiceTemplates(.init(filters: filters, offset: offset, limit: limit, needMemberInfo: needMemberInfo), region: region, logger: logger, on: eventLoop)
    }
}
