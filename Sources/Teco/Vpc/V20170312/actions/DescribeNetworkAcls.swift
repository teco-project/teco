...
extension Vpc {
    ...
    public struct DescribeNetworkAclsRequest: TCPaginatedRequest {
        ...
        public func makeNextRequest(with response: DescribeNetworkAclsResponse) -> DescribeNetworkAclsRequest? {
            guard !response.getItems().isEmpty else {
                return nil
            }
            return DescribeNetworkAclsRequest(filters: self.filters, networkAclIds: self.networkAclIds, offset: (self.offset ?? 0) + .init(response.getItems().count), limit: self.limit)
        }
    }

    /// DescribeNetworkAcls返回参数结构体
    public struct DescribeNetworkAclsResponse: TCPaginatedResponse {
        /// 实例详细信息列表。
        ...
        /// Extract the returned ``NetworkAcl`` list from the paginated response.
        public func getItems() -> [NetworkAcl] {
            self.networkAclSet
        }
    ...
    public func describeNetworkAcls(filters: [Filter]? = nil, networkAclIds: [String]? = nil, offset: UInt64? = nil, limit: UInt64? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DescribeNetworkAclsResponse> {
        self.describeNetworkAcls(.init(filters: filters, networkAclIds: networkAclIds, offset: offset, limit: limit), region: region, logger: logger, on: eventLoop)
    }
    ...
    public func describeNetworkAcls(filters: [Filter]? = nil, networkAclIds: [String]? = nil, offset: UInt64? = nil, limit: UInt64? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DescribeNetworkAclsResponse {
        try await self.describeNetworkAcls(.init(filters: filters, networkAclIds: networkAclIds, offset: offset, limit: limit), region: region, logger: logger, on: eventLoop)
    }
