...
extension Vpc {
    ...
    /// 如有需要, 可以封禁任意限速实例, 可接入到内部运营系统
    @inlinable
    public func lockCcns(logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> LockCcnsResponse {
        try await self.lockCcns(LockCcnsRequest(), logger: logger, on: eventLoop)
    }
}
