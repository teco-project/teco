...
extension Vpc {
    ...
    /// * 创建安全组同时可以绑定标签, 应答里的标签列表代表添加成功的标签。
    @inlinable
    public func createSecurityGroup(groupName: String, groupDescription: String, projectId: String? = nil, tags: [Tag]? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture < CreateSecurityGroupResponse > {
        self.createSecurityGroup(CreateSecurityGroupRequest(groupName: groupName, groupDescription: groupDescription, projectId: projectId, tags: tags), logger: logger, on: eventLoop)
    }
    ...
    /// * 创建安全组同时可以绑定标签, 应答里的标签列表代表添加成功的标签。
    @inlinable
    public func createSecurityGroup(groupName: String, groupDescription: String, projectId: String? = nil, tags: [Tag]? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> CreateSecurityGroupResponse {
        try await self.createSecurityGroup(CreateSecurityGroupRequest(groupName: groupName, groupDescription: groupDescription, projectId: projectId, tags: tags), logger: logger, on: eventLoop)
    }
}
