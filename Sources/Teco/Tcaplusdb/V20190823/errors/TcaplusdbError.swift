...
/// Service error type returned by `Tcaplusdb`.
public protocol TCTcaplusdbErrorType: TCPlatformErrorType {
    /// Get the service error as ``TCTcaplusdbError``.
    ///
    /// - Returns: ``TCTcaplusdbError`` that holds the same error code and context.
    func asTcaplusdbError() -> TCTcaplusdbError
}

public struct TCTcaplusdbError: TCTcaplusdbErrorType {
    enum Code: String {
    ...
    /// Error domains affliated to ``TCTcaplusdbError``.
    public static var domains: [TCErrorType.Type] {
        [AuthFailure.self, FailedOperation.self, InvalidParameterValue.self, ResourceInsufficient.self, ResourceUnavailable.self]
    }
