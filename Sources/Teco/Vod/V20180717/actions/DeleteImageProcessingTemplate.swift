...
extension Vod {
    ...
    /// 删除用户自定义图片处理模板。
    @inlinable
    public func deleteImageProcessingTemplate(_ input: DeleteImageProcessingTemplateRequest, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture < DeleteImageProcessingTemplateResponse > {
        self.client.execute(action: "DeleteImageProcessingTemplate", serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
