...
extension Vod {
    ...
    /// * 注意：原文件删除后，无法发起转码、微信发布等任何视频处理操作。
    @inlinable
    public func deleteMedia(fileId: String, subAppId: UInt64? = nil, deleteParts: [MediaDeleteItem]? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DeleteMediaResponse> {
        self.deleteMedia(DeleteMediaRequest(fileId: fileId, subAppId: subAppId, deleteParts: deleteParts), logger: logger, on: eventLoop)
    }
    ...
    /// * 注意：原文件删除后，无法发起转码、微信发布等任何视频处理操作。
    @inlinable
    public func deleteMedia(fileId: String, subAppId: UInt64? = nil, deleteParts: [MediaDeleteItem]? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DeleteMediaResponse {
        try await self.deleteMedia(DeleteMediaRequest(fileId: fileId, subAppId: subAppId, deleteParts: deleteParts), logger: logger, on: eventLoop)
    }
}
