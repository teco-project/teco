...
extension Vod {
    ...
    public struct DescribeAIRecognitionTemplatesRequest: TCPaginatedRequest {
        ...
        public func getNextPaginatedRequest(with response: DescribeAIRecognitionTemplatesResponse) -> DescribeAIRecognitionTemplatesRequest? {
            guard !response.getItems().isEmpty else {
                return nil
            }
            return DescribeAIRecognitionTemplatesRequest(subAppId: self.subAppId, definitions: self.definitions, offset: (self.offset ?? 0) + .init(response.getItems().count), limit: self.limit)
        }
    }
