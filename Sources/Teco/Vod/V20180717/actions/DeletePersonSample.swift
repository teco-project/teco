...
extension Vod {
    ...
    /// 该接口用于根据人物 ID，删除素材样本。
    @inlinable
    public func deletePersonSample(personId: String, subAppId: UInt64? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DeletePersonSampleResponse> {
        self.deletePersonSample(DeletePersonSampleRequest(personId: personId, subAppId: subAppId), logger: logger, on: eventLoop)
    }
    ...
    /// 该接口用于根据人物 ID，删除素材样本。
    @inlinable
    public func deletePersonSample(personId: String, subAppId: UInt64? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DeletePersonSampleResponse {
        try await self.deletePersonSample(DeletePersonSampleRequest(personId: personId, subAppId: subAppId), logger: logger, on: eventLoop)
    }
}
