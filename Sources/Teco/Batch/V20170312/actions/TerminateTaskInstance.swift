...
extension Batch {
    ...
    /// 对于状态为“FAILED_INTERRUPTED”的任务实例，终止操作实际成功之后，相关资源和配额才会释放。
    @inlinable
    public func terminateTaskInstance(jobId: String, taskName: String, taskInstanceIndex: Int64, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> TerminateTaskInstanceResponse {
        try await self.terminateTaskInstance(TerminateTaskInstanceRequest(jobId: jobId, taskName: taskName, taskInstanceIndex: taskInstanceIndex), logger: logger, on: eventLoop)
    }
}
