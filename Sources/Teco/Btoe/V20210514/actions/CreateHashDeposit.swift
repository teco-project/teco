...
extension Btoe {
    ...
    public struct CreateHashDepositRequest: TCRequestModel {
        ...
        public init(evidenceName: String, evidenceHash: String, businessId: String? = nil, evidenceDescription: String? = nil) {
    ...
    /// 用户通过本接口向BTOE写入待存证的原文数据Hash值，BTOE对业务数据Hash值存证上链，并生成含有电子签章的区块链存证电子凭证。
    @inlinable
    public func createHashDeposit(evidenceName: String, evidenceHash: String, businessId: String? = nil, evidenceDescription: String? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<CreateHashDepositResponse> {
        let input = CreateHashDepositRequest(evidenceName: evidenceName, evidenceHash: evidenceHash, businessId: businessId, evidenceDescription: evidenceDescription)
        return self.client.execute(action: "CreateHashDeposit", region: region, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
    ...
    /// 用户通过本接口向BTOE写入待存证的原文数据Hash值，BTOE对业务数据Hash值存证上链，并生成含有电子签章的区块链存证电子凭证。
    @inlinable
    public func createHashDeposit(evidenceName: String, evidenceHash: String, businessId: String? = nil, evidenceDescription: String? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> CreateHashDepositResponse {
        let input = CreateHashDepositRequest(evidenceName: evidenceName, evidenceHash: evidenceHash, businessId: businessId, evidenceDescription: evidenceDescription)
        return try await self.client.execute(action: "CreateHashDeposit", region: region, serviceConfig: self.config, input: input, logger: logger, on: eventLoop).get()
    }
}
