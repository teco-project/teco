...
extension Teo {
    ...
    /// 获取应用代理列表
    @inlinable
    public func describeApplicationProxy(zoneId: String? = nil, offset: Int64? = nil, limit: Int64? = nil, proxyId: String? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture < DescribeApplicationProxyResponse > {
        self.describeApplicationProxy(DescribeApplicationProxyRequest(zoneId: zoneId, offset: offset, limit: limit, proxyId: proxyId), logger: logger, on: eventLoop)
    }
    
    /// 获取应用代理列表
    @inlinable
    public func describeApplicationProxy(zoneId: String? = nil, offset: Int64? = nil, limit: Int64? = nil, proxyId: String? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DescribeApplicationProxyResponse {
        try await self.describeApplicationProxy(DescribeApplicationProxyRequest(zoneId: zoneId, offset: offset, limit: limit, proxyId: proxyId), logger: logger, on: eventLoop)
    }
}
