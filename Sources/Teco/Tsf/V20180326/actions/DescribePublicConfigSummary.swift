...
extension Tsf {
    ...
    public struct DescribePublicConfigSummaryRequest: TCPaginatedRequest {
        ...
        public func getNextPaginatedRequest(with response: DescribePublicConfigSummaryResponse) -> DescribePublicConfigSummaryRequest? {
            guard !response.getItems().isEmpty else {
                return nil
            }
            return DescribePublicConfigSummaryRequest(searchWord: self.searchWord, offset: (self.offset ?? 0) + .init(response.getItems().count), limit: self.limit, orderBy: self.orderBy, orderType: self.orderType, configTagList: self.configTagList, disableProgramAuthCheck: self.disableProgramAuthCheck, configIdList: self.configIdList)
        }
    }
    ...
    public func describePublicConfigSummary(searchWord: String? = nil, offset: Int64? = nil, limit: Int64? = nil, orderBy: String? = nil, orderType: Int64? = nil, configTagList: [String]? = nil, disableProgramAuthCheck: Bool? = nil, configIdList: [String]? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DescribePublicConfigSummaryResponse> {
        ...
        return self.client.execute(action: "DescribePublicConfigSummary", region: region, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
