...
extension Cwp {
    ...
    public struct DescribeAssetPortInfoListRequest: TCPaginatedRequest {
        /// 查询指定Quuid主机的信息
        ...
        public func makeNextRequest(with response: DescribeAssetPortInfoListResponse) -> DescribeAssetPortInfoListRequest? {
            ...
            return DescribeAssetPortInfoListRequest(quuid: self.quuid, filters: self.filters, limit: self.limit, offset: (self.offset ?? 0) + .init(response.getItems().count), order: self.order, by: self.by)
        }
    }

    /// DescribeAssetPortInfoList返回参数结构体
    public struct DescribeAssetPortInfoListResponse: TCPaginatedResponse {
        /// 记录总数
        ...
        /// Extract the returned ``AssetPortBaseInfo`` list from the paginated response.
        public func getItems() -> [AssetPortBaseInfo] {
            self.ports ?? []
        }
    ...
    public func describeAssetPortInfoList(quuid: String? = nil, filters: [Filter]? = nil, limit: UInt64? = nil, offset: UInt64? = nil, order: String? = nil, by: String? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DescribeAssetPortInfoListResponse> {
        self.describeAssetPortInfoList(.init(quuid: quuid, filters: filters, limit: limit, offset: offset, order: order, by: by), region: region, logger: logger, on: eventLoop)
    }
    ...
    public func describeAssetPortInfoList(quuid: String? = nil, filters: [Filter]? = nil, limit: UInt64? = nil, offset: UInt64? = nil, order: String? = nil, by: String? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DescribeAssetPortInfoListResponse {
        try await self.describeAssetPortInfoList(.init(quuid: quuid, filters: filters, limit: limit, offset: offset, order: order, by: by), region: region, logger: logger, on: eventLoop)
    }
