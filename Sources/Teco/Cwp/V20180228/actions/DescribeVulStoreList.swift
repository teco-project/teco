...
extension Cwp {
    ...
    public struct DescribeVulStoreListRequest: TCPaginatedRequest {
        ...
        public func makeNextRequest(with response: DescribeVulStoreListResponse) -> DescribeVulStoreListRequest? {
            ...
            return DescribeVulStoreListRequest(filters: self.filters, limit: self.limit, offset: (self.offset ?? 0) + .init(response.getItems().count), order: self.order, by: self.by)
        }
    }
