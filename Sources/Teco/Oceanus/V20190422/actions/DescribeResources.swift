...
extension Oceanus {
    ...
    public struct DescribeResourcesRequest: TCPaginatedRequest {
        ...
        public func makeNextRequest(with response: DescribeResourcesResponse) -> DescribeResourcesRequest? {
            ...
            return .init(resourceIds: self.resourceIds, offset: (self.offset ?? 0) + .init(response.getItems().count), limit: self.limit, filters: self.filters, workSpaceId: self.workSpaceId)
        }
    }
    ...
    public struct DescribeResourcesResponse: TCPaginatedResponse {
        ...
        /// Extract the returned ``ResourceItem`` list from the paginated response.
    ...
    /// 描述资源接口
    ///
    /// - Returns: `AsyncSequence`s of ``ResourceItem`` and ``DescribeResourcesResponse`` that can be iterated over asynchronously on demand.
    @inlinable
