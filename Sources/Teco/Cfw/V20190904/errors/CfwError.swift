...
/// Service error type returned by `Cfw`.
public protocol TCCfwErrorType: TCPlatformErrorType {
    /// Get the service error as ``TCCfwError``.
    ///
    /// - Returns: ``TCCfwError`` that holds the same error code and context.
    func asCfwError() -> TCCfwError
}

public struct TCCfwError: TCCfwErrorType {
    enum Code: String {
    ...
    public func asCfwError() -> TCCfwError {
        return self
    }
}
