...
extension Tdmq {
    ...
    public struct DescribeCmqQueuesRequest: TCPaginatedRequest {
        ...
        public func makeNextRequest(with response: DescribeCmqQueuesResponse) -> DescribeCmqQueuesRequest? {
            guard case let items = response.getItems(), !items.isEmpty, let totalCount = response.getTotalCount(), (self.offset ?? 0) + .init(items.count) >= totalCount else {
                return nil
            }
            return .init(offset: (self.offset ?? 0) + .init(response.getItems().count), limit: self.limit, queueName: self.queueName, queueNameList: self.queueNameList, isTagFilter: self.isTagFilter, filters: self.filters)
        }
    }
    ...
    public struct DescribeCmqQueuesResponse: TCPaginatedResponse {
        ...
        /// Extract the returned ``CmqQueue`` list from the paginated response.
    ...
    public func describeCmqQueues(offset: UInt64? = nil, limit: UInt64? = nil, queueName: String? = nil, queueNameList: [String]? = nil, isTagFilter: Bool? = nil, filters: [Filter]? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DescribeCmqQueuesResponse> {
        self.describeCmqQueues(.init(offset: offset, limit: limit, queueName: queueName, queueNameList: queueNameList, isTagFilter: isTagFilter, filters: filters), region: region, logger: logger, on: eventLoop)
    }
    ...
    public func describeCmqQueues(offset: UInt64? = nil, limit: UInt64? = nil, queueName: String? = nil, queueNameList: [String]? = nil, isTagFilter: Bool? = nil, filters: [Filter]? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DescribeCmqQueuesResponse {
        try await self.describeCmqQueues(.init(offset: offset, limit: limit, queueName: queueName, queueNameList: queueNameList, isTagFilter: isTagFilter, filters: filters), region: region, logger: logger, on: eventLoop)
    }
    ...
    /// 查询cmq全量队列
    ///
    /// - Returns: `AsyncSequence`s of ``CmqQueue`` and ``DescribeCmqQueuesResponse`` that can be iterated over asynchronously on demand.
    @inlinable
