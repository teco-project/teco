...
extension Tag {
    /// DescribeTags请求参数结构体
    public struct DescribeTagsRequest: TCPaginatedRequest {
    ...
    /// DescribeTags返回参数结构体
    public struct DescribeTagsResponse: TCPaginatedResponse {
        /// 结果总数
    ...
    public func describeTags(tagKey: String? = nil, tagValue: String? = nil, offset: UInt64? = nil, limit: UInt64? = nil, createUin: UInt64? = nil, tagKeys: [String]? = nil, showProject: UInt64? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DescribeTagsResponse> {
        self.describeTags(.init(tagKey: tagKey, tagValue: tagValue, offset: offset, limit: limit, createUin: createUin, tagKeys: tagKeys, showProject: showProject), region: region, logger: logger, on: eventLoop)
    }
    ...
    public func describeTags(tagKey: String? = nil, tagValue: String? = nil, offset: UInt64? = nil, limit: UInt64? = nil, createUin: UInt64? = nil, tagKeys: [String]? = nil, showProject: UInt64? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DescribeTagsResponse {
        try await self.describeTags(.init(tagKey: tagKey, tagValue: tagValue, offset: offset, limit: limit, createUin: createUin, tagKeys: tagKeys, showProject: showProject), region: region, logger: logger, on: eventLoop)
    }
    ...
    /// 用于查询已建立的标签列表。
    ///
    /// - Returns: `AsyncSequence`s of `TagWithDelete` and `DescribeTagsResponse` that can be iterated over asynchronously on demand.
    @inlinable
    public func describeTagsPaginator(_ input: DescribeTagsRequest, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> TCClient.PaginatorSequences<DescribeTagsRequest> {
        TCClient.Paginator.makeAsyncSequences(input: input, region: region, command: self.describeTags, logger: logger, on: eventLoop)
    }
}
