...
extension Sms {
    ...
    /// >- 您可以在 [API 3.0 Explorer](https://console.cloud.tencent.com/api/explorer?Product=sms&Version=2021-01-11&Action=SendSms) 中直接运行该接口，可以先免去签名计算步骤。运行成功后，API Explorer可以**自动生成**SDK代码示例。
    @inlinable
    public func pullSmsReplyStatus(limit: UInt64, smsSdkAppId: String, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> PullSmsReplyStatusResponse {
        try await self.pullSmsReplyStatus(PullSmsReplyStatusRequest(limit: limit, smsSdkAppId: smsSdkAppId), logger: logger, on: eventLoop)
    }
}
