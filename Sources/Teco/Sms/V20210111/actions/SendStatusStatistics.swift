...
extension Sms {
    ...
    /// >- 您可以在 [API 3.0 Explorer](https://console.cloud.tencent.com/api/explorer?Product=sms&Version=2021-01-11&Action=SendSms) 中直接运行该接口，可以先免去签名计算步骤。运行成功后，API Explorer可以**自动生成**SDK代码示例。
    @inlinable
    public func sendStatusStatistics(beginTime: String, endTime: String, smsSdkAppId: String, limit: UInt64, offset: UInt64, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture < SendStatusStatisticsResponse > {
        self.sendStatusStatistics(SendStatusStatisticsRequest(beginTime: beginTime, endTime: endTime, smsSdkAppId: smsSdkAppId, limit: limit, offset: offset), logger: logger, on: eventLoop)
    }
    ...
    /// >- 您可以在 [API 3.0 Explorer](https://console.cloud.tencent.com/api/explorer?Product=sms&Version=2021-01-11&Action=SendSms) 中直接运行该接口，可以先免去签名计算步骤。运行成功后，API Explorer可以**自动生成**SDK代码示例。
    @inlinable
    public func sendStatusStatistics(beginTime: String, endTime: String, smsSdkAppId: String, limit: UInt64, offset: UInt64, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> SendStatusStatisticsResponse {
        try await self.sendStatusStatistics(SendStatusStatisticsRequest(beginTime: beginTime, endTime: endTime, smsSdkAppId: smsSdkAppId, limit: limit, offset: offset), logger: logger, on: eventLoop)
    }
}
