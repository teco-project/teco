...
extension Iotcloud {
    ...
    /// 本接口（GetCOSURL）用于获取固件存储在COS的URL
    @inlinable
    public func getCOSURL(productID: String, firmwareVersion: String, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<GetCOSURLResponse> {
        let input = GetCOSURLRequest(productID: productID, firmwareVersion: firmwareVersion)
        return self.client.execute(action: "GetCOSURL", region: region, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
    ...
    /// 本接口（GetCOSURL）用于获取固件存储在COS的URL
    @inlinable
    public func getCOSURL(productID: String, firmwareVersion: String, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> GetCOSURLResponse {
        let input = GetCOSURLRequest(productID: productID, firmwareVersion: firmwareVersion)
        return try await self.client.execute(action: "GetCOSURL", region: region, serviceConfig: self.config, input: input, logger: logger, on: eventLoop).get()
    }
}
