...
extension Iotcloud {
    ...
    /// 本接口（DescribeProducts）用于列出产品列表。
    @inlinable
    public func describeProducts(offset: UInt64, limit: UInt64, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DescribeProductsResponse> {
        self.describeProducts(DescribeProductsRequest(offset: offset, limit: limit), logger: logger, on: eventLoop)
    }
    ...
    /// 本接口（DescribeProducts）用于列出产品列表。
    @inlinable
    public func describeProducts(offset: UInt64, limit: UInt64, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DescribeProductsResponse {
        try await self.describeProducts(DescribeProductsRequest(offset: offset, limit: limit), logger: logger, on: eventLoop)
    }
}
