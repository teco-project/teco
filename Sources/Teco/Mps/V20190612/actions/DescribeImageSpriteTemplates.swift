...
extension Mps {
    ...
    /// 查询雪碧图模板，支持根据条件，分页查询。
    @inlinable
    public func describeImageSpriteTemplates(definitions: [UInt64]? = nil, offset: UInt64? = nil, limit: UInt64? = nil, type: String? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture < DescribeImageSpriteTemplatesResponse > {
        self.describeImageSpriteTemplates(DescribeImageSpriteTemplatesRequest(definitions: definitions, offset: offset, limit: limit, type: type), logger: logger, on: eventLoop)
    }
    ...
    /// 查询雪碧图模板，支持根据条件，分页查询。
    @inlinable
    public func describeImageSpriteTemplates(definitions: [UInt64]? = nil, offset: UInt64? = nil, limit: UInt64? = nil, type: String? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DescribeImageSpriteTemplatesResponse {
        try await self.describeImageSpriteTemplates(DescribeImageSpriteTemplatesRequest(definitions: definitions, offset: offset, limit: limit, type: type), logger: logger, on: eventLoop)
    }
}
