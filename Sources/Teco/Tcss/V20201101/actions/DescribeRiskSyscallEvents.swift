...
extension Tcss {
    ...
    public struct DescribeRiskSyscallEventsRequest: TCPaginatedRequest {
        ...
        public func makeNextRequest(with response: DescribeRiskSyscallEventsResponse) -> DescribeRiskSyscallEventsRequest? {
            guard !response.getItems().isEmpty else {
                return nil
            }
            return DescribeRiskSyscallEventsRequest(limit: self.limit, offset: (self.offset ?? 0) + .init(response.getItems().count), filters: self.filters, order: self.order, by: self.by)
        }
    }

    /// DescribeRiskSyscallEvents返回参数结构体
    public struct DescribeRiskSyscallEventsResponse: TCPaginatedResponse {
        /// 事件总数量
        ...
        /// Extract the returned ``RiskSyscallEventInfo`` list from the paginated response.
        public func getItems() -> [RiskSyscallEventInfo] {
            self.eventSet
        }
    ...
    /// 查询运行时运行时高危系统调用列表信息
    @inlinable
    public func describeRiskSyscallEventsPaginated(_ input: DescribeRiskSyscallEventsRequest, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<(UInt64?, [RiskSyscallEventInfo])> {
        self.client.paginate(input: input, region: region, command: self.describeRiskSyscallEvents, logger: logger, on: eventLoop)
    }
