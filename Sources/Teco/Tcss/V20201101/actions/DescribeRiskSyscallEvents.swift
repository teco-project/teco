...
extension Tcss {
    ...
    /// 查询运行时运行时高危系统调用列表信息
    @inlinable
    public func describeRiskSyscallEvents(limit: UInt64? = nil, offset: UInt64? = nil, filters: [RunTimeFilters]? = nil, order: String? = nil, by: String? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture < DescribeRiskSyscallEventsResponse > {
        self.describeRiskSyscallEvents(DescribeRiskSyscallEventsRequest(limit: limit, offset: offset, filters: filters, order: order, by: by), logger: logger, on: eventLoop)
    }
    ...
    /// 查询运行时运行时高危系统调用列表信息
    @inlinable
    public func describeRiskSyscallEvents(limit: UInt64? = nil, offset: UInt64? = nil, filters: [RunTimeFilters]? = nil, order: String? = nil, by: String? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DescribeRiskSyscallEventsResponse {
        try await self.describeRiskSyscallEvents(DescribeRiskSyscallEventsRequest(limit: limit, offset: offset, filters: filters, order: order, by: by), logger: logger, on: eventLoop)
    }
}
