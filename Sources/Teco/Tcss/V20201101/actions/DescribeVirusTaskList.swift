...
extension Tcss {
    ...
    public struct DescribeVirusTaskListRequest: TCPaginatedRequest {
        ...
        public func makeNextRequest(with response: DescribeVirusTaskListResponse) -> DescribeVirusTaskListRequest? {
            ...
            return .init(taskId: self.taskId, limit: self.limit, offset: (self.offset ?? 0) + .init(response.getItems().count), filters: self.filters, by: self.by, order: self.order)
        }
    }
    ...
    public struct DescribeVirusTaskListResponse: TCPaginatedResponse {
        ...
        /// Extract the returned ``VirusTaskInfo`` list from the paginated response.
