...
extension Clb {
    ...
    public struct DescribeBlockIPListRequest: TCPaginatedRequest {
        ...
        public func makeNextRequest(with response: DescribeBlockIPListResponse) -> DescribeBlockIPListRequest? {
    ...
    /// 查询一个负载均衡所封禁的IP列表（黑名单）。（接口灰度中，如需使用请提工单）
    ///
    /// - Returns: `AsyncSequence`s of ``BlockedIP`` and ``DescribeBlockIPListResponse`` that can be iterated over asynchronously on demand.
    @inlinable
