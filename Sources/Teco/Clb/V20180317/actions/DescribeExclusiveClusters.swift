...
extension Clb {
    ...
    public func describeExclusiveClusters(limit: UInt64? = nil, offset: UInt64? = nil, filters: [Filter]? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DescribeExclusiveClustersResponse> {
        ...
        return self.client.execute(action: "DescribeExclusiveClusters", region: region, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
    ...
    /// 查询集群信息列表，支持以集群类型、集群唯一ID、集群名字、集群标签、集群内vip、集群内负载均衡唯一id、集群网络类型、可用区等条件进行检索
    @inlinable
    public func describeExclusiveClustersPaginator(_ input: DescribeExclusiveClustersRequest, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> (results: TCClient.Paginator<DescribeExclusiveClustersRequest, DescribeExclusiveClustersResponse>.ResultSequence, responses: TCClient.Paginator<DescribeExclusiveClustersRequest, DescribeExclusiveClustersResponse>.ResponseSequence) {
        let results = TCClient.Paginator<DescribeExclusiveClustersRequest, DescribeExclusiveClustersResponse>.ResultSequence(input: input, region: region, command: self.describeExclusiveClusters, logger: logger, on: eventLoop)
        let responses = TCClient.Paginator<DescribeExclusiveClustersRequest, DescribeExclusiveClustersResponse>.ResponseSequence(input: input, region: region, command: self.describeExclusiveClusters, logger: logger, on: eventLoop)
