...
/// Service error type returned by `Clb`.
public protocol TCClbErrorType: TCPlatformErrorType {
    /// Get the service error as ``TCClbError``.
    ///
    /// - Returns: ``TCClbError`` that holds the same error code and context.
    func asClbError() -> TCClbError
}

public struct TCClbError: TCClbErrorType {
    enum Code: String {
    ...
    /// Error domains affliated to ``TCClbError``.
    public static var domains: [TCErrorType.Type] {
        [FailedOperation.self, InvalidParameter.self, InvalidParameterValue.self]
    }
