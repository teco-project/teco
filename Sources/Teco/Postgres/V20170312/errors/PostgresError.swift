...
/// Service error type returned by `Postgres`.
public protocol TCPostgresErrorType: TCPlatformErrorType {
    /// Get the service error as ``TCPostgresError``.
    ///
    /// - Returns: ``TCPostgresError`` that holds the same error code and context.
    func asPostgresError() -> TCPostgresError
}

public struct TCPostgresError: TCPostgresErrorType {
    enum Code: String {
    ...
    /// Error domains affliated to ``TCPostgresError``.
    public static var domains: [TCErrorType.Type] {
        [AuthFailure.self, FailedOperation.self, InternalError.self, InvalidParameter.self, InvalidParameterValue.self, OperationDenied.self, ResourceInsufficient.self, ResourceNotFound.self, ResourceUnavailable.self, UnauthorizedOperation.self]
    }
