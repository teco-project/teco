...
extension Apigateway {
    ...
    public struct DescribeUsagePlansStatusRequest: TCPaginatedRequest {
        ...
        public func getNextPaginatedRequest(with response: DescribeUsagePlansStatusResponse) -> DescribeUsagePlansStatusRequest? {
            guard !response.getItems().isEmpty else {
                return nil
            }
            return DescribeUsagePlansStatusRequest(limit: self.limit, offset: (self.offset ?? 0) + .init(response.getItems().count), filters: self.filters)
        }
    }
