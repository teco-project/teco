...
extension Apigateway {
    ...
    public struct DescribePluginRequest: TCPaginatedRequest {
        /// 要查询的插件ID。
        ...
        public func makeNextRequest(with response: DescribePluginResponse) -> DescribePluginRequest? {
            guard !response.getItems().isEmpty else {
                return nil
            }
            return DescribePluginRequest(pluginId: self.pluginId, limit: self.limit, offset: (self.offset ?? 0) + .init(response.getItems().count))
        }
    }

    /// DescribePlugin返回参数结构体
    public struct DescribePluginResponse: TCPaginatedResponse {
        /// 插件详情。
        ...
        public func getTotalCount() -> Int64? {
            self.result.attachedApiTotalCount
        }
    }
    ...
    public func describePlugin(pluginId: String, limit: Int64? = nil, offset: Int64? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DescribePluginResponse> {
        self.describePlugin(.init(pluginId: pluginId, limit: limit, offset: offset), region: region, logger: logger, on: eventLoop)
    }
    ...
    public func describePlugin(pluginId: String, limit: Int64? = nil, offset: Int64? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DescribePluginResponse {
        try await self.describePlugin(.init(pluginId: pluginId, limit: limit, offset: offset), region: region, logger: logger, on: eventLoop)
    }
