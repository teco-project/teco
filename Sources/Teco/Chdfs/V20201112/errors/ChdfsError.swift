...
/// Service error type returned by `Chdfs`.
public protocol TCChdfsErrorType: TCPlatformErrorType {
    /// Get the service error as ``TCChdfsError``.
    ///
    /// - Returns: ``TCChdfsError`` that holds the same error code and context.
    func asChdfsError() -> TCChdfsError
}

public struct TCChdfsError: TCChdfsErrorType {
    enum Code: String {
    ...
    /// Error domains affliated to ``TCChdfsError``.
