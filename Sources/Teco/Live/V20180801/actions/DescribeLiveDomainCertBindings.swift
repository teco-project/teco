...
extension Live {
    ...
    /// 查询绑定证书的域名列表。
    @inlinable
    public func describeLiveDomainCertBindings(domainSearch: String? = nil, offset: Int64? = nil, length: Int64? = nil, domainName: String? = nil, orderBy: String? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DescribeLiveDomainCertBindingsResponse {
        try await self.describeLiveDomainCertBindings(DescribeLiveDomainCertBindingsRequest(domainSearch: domainSearch, offset: offset, length: length, domainName: domainName, orderBy: orderBy), logger: logger, on: eventLoop)
    }
}
