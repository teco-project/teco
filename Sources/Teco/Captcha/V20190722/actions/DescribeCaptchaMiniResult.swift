...
extension Captcha {
    ...
    /// 核查验证码票据结果(小程序插件)
    @inlinable
    public func describeCaptchaMiniResult(captchaType: UInt64, ticket: String, userIp: String, captchaAppId: UInt64, appSecretKey: String, businessId: UInt64? = nil, sceneId: UInt64? = nil, macAddress: String? = nil, imei: String? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DescribeCaptchaMiniResultResponse> {
        self.describeCaptchaMiniResult(DescribeCaptchaMiniResultRequest(captchaType: captchaType, ticket: ticket, userIp: userIp, captchaAppId: captchaAppId, appSecretKey: appSecretKey, businessId: businessId, sceneId: sceneId, macAddress: macAddress, imei: imei), logger: logger, on: eventLoop)
    }
    ...
    /// 核查验证码票据结果(小程序插件)
    @inlinable
    public func describeCaptchaMiniResult(captchaType: UInt64, ticket: String, userIp: String, captchaAppId: UInt64, appSecretKey: String, businessId: UInt64? = nil, sceneId: UInt64? = nil, macAddress: String? = nil, imei: String? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> DescribeCaptchaMiniResultResponse {
        try await self.describeCaptchaMiniResult(DescribeCaptchaMiniResultRequest(captchaType: captchaType, ticket: ticket, userIp: userIp, captchaAppId: captchaAppId, appSecretKey: appSecretKey, businessId: businessId, sceneId: sceneId, macAddress: macAddress, imei: imei), logger: logger, on: eventLoop)
    }
}
