...
/// Service error type returned by `Ssa`.
public protocol TCSsaErrorType: TCPlatformErrorType {
    /// Get the service error as ``TCSsaError``.
    ///
    /// - Returns: ``TCSsaError`` that holds the same error code and context.
    func asSsaError() -> TCSsaError
}

public struct TCSsaError: TCSsaErrorType {
    enum Code: String {
    ...
    /// Error domains affliated to ``TCSsaError``.
    public static var domains: [TCErrorType.Type] {
        [AuthFailure.self, InternalError.self, UnauthorizedOperation.self]
    }
