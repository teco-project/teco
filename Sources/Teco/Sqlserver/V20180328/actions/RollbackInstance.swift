...
extension Sqlserver {
    ...
    public struct RollbackInstanceRequest: TCRequest {
        ...
        public init(instanceId: String, type: UInt64, time: Date, dBs: [String]? = nil, targetInstanceId: String? = nil, renameRestore: [RenameRestoreDatabase]? = nil) {
    ...
    /// 本接口（RollbackInstance）用于按照时间点回档实例
    @inlinable
    public func rollbackInstance(instanceId: String, type: UInt64, time: Date, dBs: [String]? = nil, targetInstanceId: String? = nil, renameRestore: [RenameRestoreDatabase]? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<RollbackInstanceResponse> {
        self.rollbackInstance(.init(instanceId: instanceId, type: type, time: time, dBs: dBs, targetInstanceId: targetInstanceId, renameRestore: renameRestore), region: region, logger: logger, on: eventLoop)
    }
    ...
    /// 本接口（RollbackInstance）用于按照时间点回档实例
    @inlinable
    public func rollbackInstance(instanceId: String, type: UInt64, time: Date, dBs: [String]? = nil, targetInstanceId: String? = nil, renameRestore: [RenameRestoreDatabase]? = nil, region: TCRegion? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> RollbackInstanceResponse {
        try await self.rollbackInstance(.init(instanceId: instanceId, type: type, time: time, dBs: dBs, targetInstanceId: targetInstanceId, renameRestore: renameRestore), region: region, logger: logger, on: eventLoop)
    }
}
