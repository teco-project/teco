...
extension Soe {
    ...
    @inlinable
    public func keywordEvaluate(_ input: KeywordEvaluateRequest, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<KeywordEvaluateResponse> {
        self.client.execute(action: "KeywordEvaluate", serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
    ...
    /// 指定主题关键词词汇评估，分析语音与关键词的切合程度，可指定多个关键词，支持中文英文同时评测。分片传输时，尽量保证纯异步调用，即不等待上一个分片的传输结果边录边传，这样可以尽可能早的提供音频数据。音频源目前仅支持16k采样率16bit单声道编码方式，如有不一致可能导致评估不准确或失败。
    @inlinable
    public func keywordEvaluate(_ input: KeywordEvaluateRequest, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> KeywordEvaluateResponse {
        try await self.client.execute(action: "KeywordEvaluate", serviceConfig: self.config, input: input, logger: logger, on: eventLoop).get()
    }
    ...
    /// 指定主题关键词词汇评估，分析语音与关键词的切合程度，可指定多个关键词，支持中文英文同时评测。分片传输时，尽量保证纯异步调用，即不等待上一个分片的传输结果边录边传，这样可以尽可能早的提供音频数据。音频源目前仅支持16k采样率16bit单声道编码方式，如有不一致可能导致评估不准确或失败。
    @inlinable
    public func keywordEvaluate(seqId: UInt64, isEnd: UInt64, voiceFileType: UInt64, voiceEncodeType: UInt64, userVoiceData: String, sessionId: String, keywords: [Keyword], soeAppId: String? = nil, isQuery: UInt64? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<KeywordEvaluateResponse> {
        self.keywordEvaluate(KeywordEvaluateRequest(seqId: seqId, isEnd: isEnd, voiceFileType: voiceFileType, voiceEncodeType: voiceEncodeType, userVoiceData: userVoiceData, sessionId: sessionId, keywords: keywords, soeAppId: soeAppId, isQuery: isQuery), logger: logger, on: eventLoop)
    }
    ...
    /// 指定主题关键词词汇评估，分析语音与关键词的切合程度，可指定多个关键词，支持中文英文同时评测。分片传输时，尽量保证纯异步调用，即不等待上一个分片的传输结果边录边传，这样可以尽可能早的提供音频数据。音频源目前仅支持16k采样率16bit单声道编码方式，如有不一致可能导致评估不准确或失败。
    @inlinable
    public func keywordEvaluate(seqId: UInt64, isEnd: UInt64, voiceFileType: UInt64, voiceEncodeType: UInt64, userVoiceData: String, sessionId: String, keywords: [Keyword], soeAppId: String? = nil, isQuery: UInt64? = nil, logger: Logger = TCClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> KeywordEvaluateResponse {
        try await self.keywordEvaluate(KeywordEvaluateRequest(seqId: seqId, isEnd: isEnd, voiceFileType: voiceFileType, voiceEncodeType: voiceEncodeType, userVoiceData: userVoiceData, sessionId: sessionId, keywords: keywords, soeAppId: soeAppId, isQuery: isQuery), logger: logger, on: eventLoop)
    }
}
